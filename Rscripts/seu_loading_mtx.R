...
seu <- LoadSeuratRds("/mnt/ix1/Projects_lite/20240603_BC_P8640/scRNA/C01_Combined/01_flames_analysis/SR_seu_backup.rds")
setwd("/mnt/ix1/Projects_lite/20240603_BC_P8640/scRNA/C01_Combined/02_custom_analysis")
dest_dir='/mnt/ix1/Projects_lite/20240603_BC_P8640/scRNA/C01_Combined/02_custom_analysis/master.R_out'

# 11. Load long read data ------------------------------------------------
longread_matrix <- read.csv("/mnt/ix1/Projects_lite/20240603_BC_P8640/scRNA/C01_Combined/02_custom_analysis/output_matrix.csv",
                            header = TRUE,
                            check.names = FALSE)

seu

# For some reason there is an extra row and column, so filtering those out.
longread_matrix <- longread_matrix[, colnames(longread_matrix) != '#read_1']
longread_matrix <- longread_matrix[-1,]

# Change _ to dash
colnames(longread_matrix) <- gsub("_", "-", colnames(longread_matrix), fixed = TRUE)

# Remove rownames, assign to correct location
rownames(longread_matrix) <- longread_matrix[,1]
longread_matrix[,1] <- NULL



## 11.1 Extract barcodes from Seu obj and LR --------------------------------
short_barcodes <- colnames(seu)
#longread_matrix  <- longread_matrix %>% tibble::column_to_rownames(var="transcript_id")
...
saveRDS(seu, file = "CA_SR_LR_seu.rds")
# Keep saved copy separate
seu
...
ggsave(file.path(dest_dir,"SR_LR_UMAP.png"), plot = new_merged_plot,width = 14, height=8)
